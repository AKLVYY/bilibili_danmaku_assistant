...
def get_qrcode():
    qrcode_response = requests.get('https://passport.bilibili.com/x/passport-login/web/qrcode/generate', headers=HEADERS)
    qrcode_key = qrcode_response.json()['data']['qrcode_key']
    qrcode_url = qrcode_response.json()['data']['url']
...
def poll_if_scan(qrcode_key: str) -> Literal["Success", "Waiting", "Confirming", "Timeout"]:
    ...
    poll_response = requests.get(base_url, headers=HEADERS, params=params)
    if poll_response.json()['data']['code'] == 0:
        with open(COOKIE_PATH, 'w', encoding='utf-8') as f:
            json.dump(requests.utils.dict_from_cookiejar(poll_response.cookies), f)
        return "Success"
    elif poll_response.json()['data']['code'] == 86101:
        return "Waiting"
    elif poll_response.json()['data']['code'] == 86090:
        return "Confirming"
    elif poll_response.json()['data']['code'] == 86038:
        return "Timeout"
